...
(function () {
    ...
    function applyServerState(online) {
        ...
        if (badge) {
            ...
            badge.querySelector('.txt').textContent = online ? 'Server online' : 'Server offline';
        }
    ...
    function wireTool(opts) {
        ...
        runBtn.addEventListener('click', async function () {
            ...
            if (!serverOnline) { setStatus(statusId, 'Cannot reach the server. Please try again in a moment.', 'err'); return; }
            // field validation
            if (opts.validate) {
                var v = opts.validate();
            ...
            setStatus(statusId, 'Processing on server…');
            try {
                var fd = new FormData();
                ...
                if (opts.expect === 'text') {
                    ...
                    setStatus(statusId, 'Done · ' + result.text.length.toLocaleString() + ' characters recognized', 'ok');
                } else {
                    var fname = filenameFromResponse(result.res, opts.outName(state.file));
                    download(result.blob, fname);
                    setStatus(statusId, 'Done · ' + fmtBytes(result.blob.size) + ' saved', 'ok');
                }
            } catch (e) {
                console.error(e);
    ...
    (function () {
        ...
        if (copyBtn) copyBtn.addEventListener('click', function () {
            ...
            navigator.clipboard.writeText(lastText).then(function () {
                if (window.LD && LD.toast) LD.toast('Copied to clipboard.');
            }, function () {
    ...
    (function () {
        ...
        wireTool({
            ...
            validate: function () { return (pw && pw.value) ? '' : 'Enter a password.'; },
    ...
    (function () {
        ...
        wireTool({
            ...
            validate: function () {
                ...
                if (!r || !c || r < 1 || c < 1) return 'Rows and columns must both be at least 1.';
                return '';
            },
    ...
    (function () {
        ...
        wireTool({
            ...
            validate: function () { return (txt && txt.value) ? '' : 'Enter watermark text.'; },
