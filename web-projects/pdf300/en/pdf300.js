...
(function () {
    ...
    document.getElementById('merge-run').addEventListener('click', async function () {
        setStatus('merge-status', 'Processing…');
        try {
            ...
            setStatus('merge-status', 'Done · ' + LD.formatBytes(bytes.byteLength) + ' saved', 'ok');
        } catch (e) { setStatus('merge-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('split-drop', 'split-file', async function (files) {
        ...
        setStatus('split-info', files[0].name + ' · ' + splitState.pageCount + ' pages', 'ok');
        document.getElementById('split-range').placeholder = 'e.g. 1-' + splitState.pageCount;
    ...
    document.getElementById('split-run').addEventListener('click', async function () {
        ...
        try {
            ...
            if (mode === 'range') {
                ...
                if (!groups.length) throw new Error('No valid page range.');
            } else if (mode === 'each') {
            ...
            setStatus('split-status', 'Done · ' + groups.length + ' files', 'ok');
        } catch (e) { setStatus('split-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('org-drop', 'org-file', async function (files) {
        ...
        setStatus('org-status', 'Generating preview…');
        try {
            var buf = await readArrayBuffer(files[0]);
            orgState.buf = buf;
            ...
            for (var i = 1; i <= pdf.numPages; i++) {
                ...
                thumb.insertAdjacentHTML('beforeend',
                    ...
                    '<button class="preview-thumb-action" data-rot="-90" title="Rotate left">↺</button>' +
                    '<button class="preview-thumb-action" data-rot="90" title="Rotate right">↻</button>' +
                    '<button class="preview-thumb-action" data-toggle title="Exclude">✕</button>' +
                    '</div>');
                grid.appendChild(thumb);
                bindOrgThumb(thumb, i - 1);
            }
            document.getElementById('org-run').disabled = false;
            setStatus('org-status', orgState.file.name + ' · ' + pdf.numPages + ' pages', 'ok');
        } catch (e) { setStatus('org-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    document.getElementById('org-run').addEventListener('click', async function () {
        ...
        try {
            ...
            setStatus('org-status', 'Done · ' + keep.length + ' pages', 'ok');
        } catch (e) { setStatus('org-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('crop-drop', 'crop-file', async function (files) {
        ...
        setStatus('crop-status', 'Generating preview…');
        try {
            var buf = await readArrayBuffer(files[0]);
            cropState.buf = buf;
        ...
        } catch (e) { setStatus('crop-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    document.getElementById('crop-run').addEventListener('click', async function () {
        ...
        setStatus('crop-status', 'Cropping…');
        try {
            ...
            setStatus('crop-status', 'Done · ' + LD.formatBytes(bytes.byteLength), 'ok');
        } catch (e) { setStatus('crop-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('nup-drop', 'nup-file', async function (files) {
        ...
        nupState.buf = buf;
        var doc = await PDFDocument.load(buf, { ignoreEncryption: true });
        setStatus('nup-info', files[0].name + ' · ' + doc.getPageCount() + ' pages', 'ok');
    ...
    document.getElementById('nup-run').addEventListener('click', async function () {
        ...
        setStatus('nup-status', 'Rebuilding…');
        try {
            ...
            setStatus('nup-status', 'Done · ' + LD.formatBytes(bytes.byteLength), 'ok');
        } catch (e) { console.error(e); setStatus('nup-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('num-drop', 'num-file', async function (files) {
        ...
        numState.buf = buf;
        var doc = await PDFDocument.load(buf, { ignoreEncryption: true });
        setStatus('num-info', files[0].name + ' · ' + doc.getPageCount() + ' pages', 'ok');
    ...
    document.getElementById('num-run').addEventListener('click', async function () {
        ...
        setStatus('num-status', 'Adding page numbers…');
        try {
            ...
            setStatus('num-status', 'Done · ' + N + ' pages', 'ok');
        } catch (e) { setStatus('num-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('wm-drop', 'wm-file', async function (files) {
        ...
        wmState.buf = buf;
        var doc = await PDFDocument.load(buf, { ignoreEncryption: true });
        setStatus('wm-info', files[0].name + ' · ' + doc.getPageCount() + ' pages', 'ok');
    ...
    document.getElementById('wm-run').addEventListener('click', async function () {
        ...
        setStatus('wm-status', 'Adding watermark…');
        try {
            ...
            setStatus('wm-status', 'Done', 'ok');
        } catch (e) { setStatus('wm-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('sign-drop', 'sign-file', async function (files) {
        ...
        setStatus('sign-status', 'Generating preview…');
        try {
            var buf = await readArrayBuffer(files[0]);
            signState.buf = buf;
        ...
        } catch (e) { setStatus('sign-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    document.getElementById('sign-overlay').addEventListener('click', function (e) {
        if (signState.currentSig === null) {
            LD.toast('Pick a signature from the tray first.', 'error');
            return;
        }
    ...
    document.getElementById('sign-run').addEventListener('click', async function () {
        ...
        setStatus('sign-status', 'Placing signature…');
        try {
            ...
            setStatus('sign-status', 'Done · ' + signState.placements.length + ' placements', 'ok');
        } catch (e) { console.error(e); setStatus('sign-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('rd-drop', 'rd-file', async function (files) {
        ...
        setStatus('rd-status', 'Generating preview…');
        try {
            var buf = await readArrayBuffer(files[0]);
            rdState.buf = buf;
            ...
            setStatus('rd-status', '');
        } catch (e) { setStatus('rd-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    document.getElementById('rd-run').addEventListener('click', async function () {
        ...
        try {
            ...
            setStatus('rd-status', 'Done · ' + rdState.boxes.length + ' areas', 'ok');
        } catch (e) { setStatus('rd-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('ext-drop', 'ext-file', async function (files) {
        ...
        grid.innerHTML = '<div class="status">Generating preview…</div>';
        try {
            var buf = await readArrayBuffer(files[0]);
            ...
            if (pdf.numPages > 30) {
                grid.insertAdjacentHTML('beforeend', '<div class="status">+' + (pdf.numPages - 30) + ' more pages</div>');
            }
            document.getElementById('ext-run').disabled = false;
            setStatus('ext-status', extFile.name + ' · ' + pdf.numPages + ' pages', 'ok');
        } catch (e) { setStatus('ext-status', 'Error: ' + e.message, 'err'); }
    });
    document.getElementById('ext-run').addEventListener('click', async function () {
        if (!extFile) return;
        setStatus('ext-status', 'Converting…');
        try {
            ...
            for (var i = 1; i <= pdf.numPages; i++) {
                setStatus('ext-status', 'Converting… (' + i + '/' + pdf.numPages + ')');
            ...
            setStatus('ext-status', 'Done · ' + pdf.numPages + ' images', 'ok');
        } catch (e) { setStatus('ext-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    document.getElementById('img-run').addEventListener('click', async function () {
        ...
        try {
            ...
            setStatus('img-status', 'Done · ' + imgFiles.length + ' pages', 'ok');
        } catch (e) { setStatus('img-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('txt-drop', 'txt-file', function (files) {
        ...
        setStatus('txt-status', txtFile.name + ' ready', 'ok');
    });
    document.getElementById('txt-run').addEventListener('click', async function () {
        if (!txtFile) return;
        setStatus('txt-status', 'Extracting text…');
        try {
            ...
            var pages = [];
            for (var i = 1; i <= pdf.numPages; i++) {
                setStatus('txt-status', 'Extracting… (' + i + '/' + pdf.numPages + ')');
                var page = await pdf.getPage(i);
            ...
            setStatus('txt-status', 'Done · ' + pdf.numPages + ' pages · ' + txtResult.length + ' chars', 'ok');
        } catch (e) { setStatus('txt-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    document.getElementById('cmp-run').addEventListener('click', async function () {
        ...
        setStatus('cmp-status', 'Compressing…');
        try {
            ...
            var out = await PDFDocument.create();
            for (var i = 1; i <= pdf.numPages; i++) {
                setStatus('cmp-status', 'Compressing… (' + i + '/' + pdf.numPages + ')');
                var page = await pdf.getPage(i);
                var vp = page.getViewport({ scale: 1 });
                var scale = Math.min(2, maxW / vp.width);
                var vp2 = page.getViewport({ scale: scale });
            ...
            setStatus('cmp-status',
                'Done · ' + LD.formatBytes(origSize) + ' → ' + LD.formatBytes(bytes.byteLength) +
                ' (' + (saved >= 0 ? '−' : '+') + Math.abs(pct) + '%)',
                saved >= 0 ? 'ok' : 'err');
        } catch (e) { setStatus('cmp-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    bindDrop('meta-drop', 'meta-file', async function (files) {
        ...
        try {
            ...
            var info = {
                ...
                'CreationDate': (doc.getCreationDate() || '').toString().slice(0, 25) || '(none)',
                'ModificationDate': (doc.getModificationDate() || '').toString().slice(0, 25) || '(none)',
        ...
        } catch (e) { setStatus('meta-status', 'Error: ' + e.message, 'err'); }
    });
    ...
    document.getElementById('meta-strip').addEventListener('click', async function () {
        ...
        try {
            ...
            setStatus('meta-status', 'Done · metadata removed', 'ok');
        } catch (e) { setStatus('meta-status', 'Error: ' + e.message, 'err'); }
    });
