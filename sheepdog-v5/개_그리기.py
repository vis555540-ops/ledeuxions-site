...
def dog(P, head=(0,0), body_dy=0, legs=None, sit=False, eyes='open', mouth='closed', tail=(0,0), ears=0, bark=False):
    ...
    # 🚨 2026-09-12 형: 「처음 들어갈 때 있는 개도 이상하고」
    #    눈을 외곽선 색으로만 찍어서 보더콜리·셰퍼드처럼 머리가 어두운 개는 눈이 묻혔다.
    #    남는 게 분홍 볼터치뿐이라 그게 눈처럼 보였다. 흰자를 한 칸 깔아 준다.
    EW = (246, 244, 238)
    if eyes == 'open':
        rect(Hd, 6, 11, 7, 12, EW); rect(Hd, 11, 11, 12, 12, EW)
        rect(Hd, 7, 11, 7, 12, OUT); rect(Hd, 12, 11, 12, 12, OUT)
    else:
        for x in (6, 7): put(Hd, x, 11, EW)
        for x in (12, 13): put(Hd, x, 11, EW)
