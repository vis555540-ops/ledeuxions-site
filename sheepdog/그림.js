...
const 그림 = {
  ...
  시작(canvas) {
    ...
    // ★ 2026-09-25 골든리트리버 새 그림(형이 Gemini 로 뽑은 시트 → 32칸). 못 불러오면 옛 시트 그대로
    불러올.push(["개_골든리트리버_새","개/golden.png"], ["개_골든리트리버_뒤","개/golden_back.png"]);
  ...
  시트그리기(키, 규격이름, 동작, 프레임시간, x, y, 왼쪽, 팔레트색, 옵션) {
    ...
    // ★ 2026-09-25 형 「재미나이한테 받은 것만」 — 새 그림이 있으면(지금은 골든만) 아기도 그 그림을 줄여 쓴다.
    //   옛 아기 시트를 먼저 쓰면 아기 골든만 옛 그림이라 섞여 보였다
    if (그림.시트[키+"_새"]) 쓸키 = 키+"_새";
    else if (크기 < 1 && 그림.시트[키+"_아기"]) { 쓸키 = 키+"_아기"; 크기 = 1; }
  ...
  // 뒷모습(걸어가는 모습) — 산책 화면. 시트가 없으면 false 를 돌려주고 옛 방식으로 그린다
  뒷모습그리기(개, t, x, y) {
    const im = 그림.시트["개_"+개.견종+"_뒤"]; if (!im || !im.width) return false;
    let 쓸 = im; if (개.털색) 쓸 = 그림.색치환("개_"+개.견종+"_뒤", 개.털색) || im;
